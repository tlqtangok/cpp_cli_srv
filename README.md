...
All responses use the unified JSON format: `{"code": 0, "output": "...", "error": ""}`

### Schema & Info

| Endpoint | Method | Description | Auth |
|----------|--------|-------------|------|
| `/get/schema` | GET | Returns all available commands with argument definitions | No |
| `/get/version` | GET | Returns build version (date + git commit) | No |
| `/get/status` | GET | Returns server status (active requests, threads) | No |
| `/get/token` | GET | Returns server's security token | No |
...
| 5 | Server error | 500 |
| 6 | Authentication failed (invalid token) | 403 |
...
{
  "code": 0,
  "output": "result data (string, number, object, array, or boolean)",
  "error": "error message if code != 0, empty otherwise"
}
```

**Note**: `output` field is automatically parsed as JSON when the stored value is an object, array, string, number, or boolean. Arbitrary text (e.g., from shell commands) is returned as-is.
